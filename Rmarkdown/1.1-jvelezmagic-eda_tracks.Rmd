...
experiment_datasets <- tracks_summary_df %>% 
  select(experiment_id, survived_strict, contains("length"), contains("gfp")) %>% 
  group_by(experiment_id) %>% 
  {
    grouped_data <- .
...
```{r}
c_pca_rec <- recipe(survived_strict ~ ., data = chromosome_df) %>% 
...
```{r chromosome_pca_absolute_contributions}
c_tidied_pca %>%
...
c_pca_prep %>% 
  juice() %>% 
  plot_ly(x = ~PC1, y = ~PC2, z = ~PC3, color = ~survived_strict, opacity = 1/20)
```

#### Plasmid

```{r}
p_pca_rec <- recipe(survived_strict ~ ., data = plasmid_df) %>% 
  step_naomit(all_predictors()) %>% 
  step_normalize(all_predictors()) %>%
  step_pca(all_predictors())
...
```{r warning=FALSE}
chromosome_df %>% 
  select(survived_strict, where(is.numeric)) %>% 
  ggpairs(
    data = .,
    columns = 2:ncol(.),
    aes(color = survived_strict, alpha = 1/1000),
