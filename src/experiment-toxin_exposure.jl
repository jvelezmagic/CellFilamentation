...
function toxin_exposure_experiment(
    ...
    for amount_toxin ∈ amount_toxins, experiment ∈ experiments
        ...
        # Setout.
        cell_to_evaluate = experiment == :Control ? Cell(cell; filamentation_rate=0.0) : cell
        ...
        if n_cells > 1
            ...
            for n_cell ∈ 1:n_cells
                sol, sol_cell = simulate(cell_to_evaluate, time, toxin; stochastic=true, status=false, kwargs...)
                status = _check_cell_status(sol_cell, exposure_times)
                states += map(x-> x ∈ [:Normal, :Stressed] ? 1.0 : 0.0, status)
            end
            states ./= n_cells
        else
            sol, sol_cell = simulate(cell_to_evaluate, time, toxin; sthocastic=false, status=false, kwargs...)
            states = _check_cell_status(sol_cell, exposure_times)
        end
        push!(dfs, DataFrame(exposure_time=exposure_times, amount_toxin=amount_toxin, state=states, experiment=experiment))
    end

    return reduce(vcat, dfs)
end

function plot_toxin_exposure_experiment(
    df,
    width=300,
    height=300
)
    ...
    df[!, :experiment] = map(x-> x == :Control ? "Without filamentation" : "Filamentation", df[!, :experiment])
